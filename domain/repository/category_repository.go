...
// FindCategoryByLevel 获取结果集
func (u *CategoryRepository) FindCategoryByLevel(level uint32) (categoryList []model.Category, err error) {
	return categoryList, u.mysqlDb.Where("category_level = ?", level).Find(categoryList).Error
}

// FindCategoryByParent 获取结果集
func (u *CategoryRepository) FindCategoryByParent(parent int64) (categoryList []model.Category, err error) {
	return categoryList, u.mysqlDb.Where("category_parent = ?", parent).Find(categoryList).Error
}
